...
describe "When I visit a shelter show page I see a link to update" do
  it "When I click the link 'Update Shelter' I'm taken to '/shelters/:id/edit' where I see a form to edit the shelter's data" do
    ...
    fill_in "name", with: "Denver Shelter South"

    expect(find_field('name').value).to eq"Denver Shelter South"
    expect(find_field('address').value).to eq"#{shelter.address}"
    expect(find_field('city').value).to eq"#{shelter.city}"
    expect(find_field('state').value).to eq"#{shelter.state}"
    expect(find_field('zip').value).to eq"#{shelter.zip}"
  end
